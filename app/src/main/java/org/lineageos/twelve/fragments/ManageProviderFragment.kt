...
class ManageProviderFragment : Fragment(R.layout.fragment_manage_provider) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    viewModel.providerInstance.collectLatest {
                        when (it) {
                            ...
                            is RequestStatus.Error -> {
                                Log.e(LOG_TAG, "Failed to load provider instance")
                ...
                launch {
                    viewModel.providerTypeWithArguments.collectLatest { providerTypeWithArguments ->
                        val (providerType, providerInstanceArguments) = providerTypeWithArguments

                        if (providerType != selectedProviderType) {
                            // Clear the provided arguments regardless since they aren't
                            // valid anymore
                            providerArguments.clear()
                        }

                        providerInstanceArguments?.let {
                            // Load the values from the database as defaults
                            providerArguments.clear()
                            providerArguments.putAll(it)
                        }
    ...
    companion object {
         ...
         *   instance or the type of the provider instance to edit or delete
         * @param providerTypeId The ID of the provider instance to edit or delete
         */
        fun createBundle(
