...
class SubsonicDataSource(arguments: Bundle) : MediaDataSource {
    ...
    override fun playlists() = suspend {
    ...
    override fun audioPlaylistsStatus(audioUri: Uri) = flowOf(
        RequestStatus.Error<List<Pair<Playlist, Boolean>>>(RequestStatus.Error.Type.NOT_IMPLEMENTED)
    )
    ...
    override suspend fun removeAudioFromPlaylist(
        playlistUri: Uri,
        audioUri: Uri
    ) = RequestStatus.Error<Unit>(RequestStatus.Error.Type.NOT_IMPLEMENTED)
    ...
    private fun <T : Any, O : Any> SubsonicClient.MethodResult<T>.toRequestStatus(
        resultGetter: T.() -> O
    ) = when (this) {
